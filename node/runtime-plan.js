    ...
    return {
        ...
        operations: operations.map((op) => {
            ...
            return {
                ...
                ...(body ? { body: root(body) } : {}),
                responses: Object.fromEntries(Object.entries(responses).map(([status, { schema, ...response }]) => [
                    status,
                    {
                        ...response,
        ...
        ...(definitions
            ? {
                definitions: Object.fromEntries(Object.entries(definitions).map(([name, schema]) => [name, compileCodec(schema)])),
            }
            : {}),
        ...(webhook
            ? {
                webhook: {
                    ...webhook,
                    events: Object.fromEntries(Object.entries(webhook.events).map(([name, schema]) => [name, root(schema)])),
                },
            }
            : {}),
    };
...
export function assertRuntimePlan(value) {
    ...
    if (plan.incoming !== undefined) {
        if (!Array.isArray(plan.incoming))
            throw new Error('Invalid incoming contracts');
        for (const declaration of plan.incoming) {
            ...
            for (const key of ['name', 'method', 'pointer', 'model'])
                if (typeof entry[key] !== 'string')
                    throw new Error('Invalid incoming ' + key);
            if (entry.schema !== undefined)
                throw new Error('Raw schema in incoming contract');
    ...
    for (const [index, value] of plan.operations.entries()) {
        ...
        if (op.authModes !== undefined &&
            (!Array.isArray(op.authModes) ||
                !op.authModes.every((name) => typeof name === 'string' &&
                    plan.authentication &&
        ...
        if (op.body !== undefined)
            assertCodecPlan(op.body, `${op.id}.body`);
        ...
        if (op.streamEventCodecs !== undefined)
            for (const [event, codec] of Object.entries(record(op.streamEventCodecs, `${op.id}.streamEventCodecs`)))
        ...
        for (const [status, response] of Object.entries(record(op.responses, `${op.id}.responses`))) {
            ...
            if (result.variants !== undefined)
                for (const binding of Object.values(record(result.variants, `${op.id}.responses.${status}.variants`)))
    ...
    if (plan.webhook !== undefined)
        for (const [name, codec] of Object.entries(record(record(plan.webhook, 'webhook').events, 'webhook.events')))
            assertCodecPlan(codec, 'webhook.events.' + name);
}
//# sourceMappingURL=runtime-plan.js.map