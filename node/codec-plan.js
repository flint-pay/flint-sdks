...
export function valueInstruction(type, format) {
    switch (type) {
        case undefined:
            return { kind: 'dynamic' };
        case 'null':
...
export function wireKind(value) {
    switch (value.kind) {
        case 'dynamic':
        case 'null-array':
            return undefined;
...
function invalidInstruction(value) {
...
export function directionalSchema(schema, response) {
    const omitted = new Set();
    const collect = (s) => {
        for (const [key, child] of Object.entries(s.properties ?? {}))
            if (response ? child.writeOnly : child.readOnly)
                omitted.add(key);
...
export function discriminatorBindings(schema) {
    const property = schema.discriminator?.propertyName;
    if (!property || !schema.oneOf)
        return undefined;
    const conjuncts = (s) => [s, ...(s.allOf ?? []).flatMap(conjuncts)];
    const { oneOf, ...siblings } = schema;
    const result = Object.create(null);
    for (const [index, branch] of oneOf.entries()) {
        const parts = [...conjuncts(siblings), ...conjuncts(branch)];
        if (!parts.some((s) => s.type === 'object') ||
            !parts.some((s) => s.required?.includes(property)))
            return undefined;
        const fields = parts.flatMap((s) => s.properties?.[property] ? conjuncts(s.properties[property]) : []);
        const sets = fields.flatMap((s) => Object.hasOwn(s, 'const') ? [[s.const]] : s.enum ? [s.enum] : []);
        if (!sets.length)
            return undefined;
        const tags = sets[0]?.filter((value) => sets.every((values) => values.includes(value)));
        if (!tags?.length || tags.some((value) => typeof value !== 'string'))
            return undefined;
        for (const tag of tags) {
            if (typeof tag !== 'string' || Object.hasOwn(result, tag))
                return undefined;
            result[tag] = index;
        }
    }
...
function compileNode(input, output, depth) {
    if (depth > 256)
        throw new Error('Schema exceeds the supported compilation depth (256)');
    const request = directionalSchema(input, false);
    const conjunctiveObject = (s) => s.type === 'object' || Boolean(s.allOf?.some(conjunctiveObject));
    ...
    const branches = (a, b) => a.map((child, index) => compileNode(child, b[index] ?? child, depth + 1));
    return {
        ...(input['x-sdk-number-input'] === 'explicit' ? { numberInput: 'explicit' } : {}),
        value: Array.isArray(input.type) && !input.type.some((type) => type !== 'null')
            ? { kind: 'null-array' }
        ...
        modelObjectInput: conjunctiveObject(input) ||
        ...
        requiredOutput: (response.required ?? []).filter((key) => !response.properties?.[key]?.writeOnly),
        ...
        ...(input.properties
            ? {
                fields: Object.fromEntries(Object.entries(input.properties).map(([name, child]) => [
                    name,
        ...
        ...(input.additionalProperties !== undefined
            ? {
                extra: typeof input.additionalProperties === 'object'
        ...
        ...(input.pattern !== undefined
            ? {
                phpPattern: input['x-sdk-pattern-php'] ?? '~' + input.pattern.replaceAll('~', '\\~') + '~uD',
            }
            : {}),
        ...
        ...(request.oneOf
            ? {
                exactlyOne: branches(request.oneOf, response.oneOf ?? request.oneOf).map((branch, index) => ({
                    ...branch,
                    ...(bindings
        ...
        ...(input['x-sdk-definitions']
            ? {
                definitions: Object.fromEntries(Object.entries(input['x-sdk-definitions']).map(([name, s]) => [
                    name,
...
export function assertCodecPlan(value, path = 'codec', depth = 0) {
    ...
    if (op.kind === 'opaque' && typeof op.label !== 'string')
    ...
    for (const [key, item] of Object.entries(node.checks)) {
        if (key === 'uniqueItems') {
        ...
        if (['minProperties', 'maxProperties'].includes(key) &&
    ...
    if (node.range !== undefined &&
        (!Array.isArray(node.range) ||
            node.range.length !== 2 ||
            !node.range.every((v) => typeof v === 'string' && /^-?\d+$/.test(v))))
        invalid('invalid numeric range');
    if (node.members !== undefined && !Array.isArray(node.members))
        invalid('invalid enum members');
    ...
    for (const key of ['element', 'exclude', 'includes'])
        if (node[key] !== undefined)
            assertCodecPlan(node[key], path + '.' + key, depth + 1);
...
//# sourceMappingURL=codec-plan.js.map